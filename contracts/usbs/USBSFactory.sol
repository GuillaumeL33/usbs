 ...
 * @title USBSFactory
 * @author Brivo
 * @notice This contract serves as a Factory for the upgradable USBS token contract.
 *         Upon calling `deployUSBS` the `guardian` address (set in constructor) will
 *         deploy the following:
 *         1) USBS - The implementation contract, ERC20 contract with the initializer disabled
 ...
 *
 *         Following the above mentioned deployment, the address of the USBSFactory contract will:
 ...
 */
contract USBSFactory is IMulticall {
  struct USBSListData {
    address blocklist;
    address allowlist;
  ...
  USBS public usbsImplementation;
  ProxyAdmin public usbsProxyAdmin;
  TokenProxy public usbsProxy;
  ...
  /**
   * @dev This function will deploy an upgradable instance of USBS
   *
  ...
  function deployUSBS(
    ...
    USBSListData calldata listData
  ) external onlyGuardian returns (address, address, address) {
    usbsImplementation = new USBS();
    usbsProxyAdmin = new ProxyAdmin();
    usbsProxy = new TokenProxy(
      address(usbsImplementation),
      address(usbsProxyAdmin),
      ""
    );
    USBS usbsProxied = USBS(address(usbsProxy));
    usbsProxied.initialize(
      name,
      ticker,
      listData.blocklist,
      listData.allowlist,
    ...
    usbsProxied.grantRole(DEFAULT_ADMIN_ROLE, guardian);
    usbsProxied.grantRole(PAUSER_ROLE, guardian);

    usbsProxied.revokeRole(MINTER_ROLE, address(this));
    usbsProxied.revokeRole(PAUSER_ROLE, address(this));
    usbsProxied.revokeRole(DEFAULT_ADMIN_ROLE, address(this));

    usbsProxyAdmin.transferOwnership(guardian);
    assert(usbsProxyAdmin.owner() == guardian);
    emit USBSDeployed(
      address(usbsProxied),
      address(usbsProxyAdmin),
      address(usbsImplementation),
  ...
  /**
   * @dev Event emitted when upgradable USBS is deployed
   *
  ...
  event USBSDeployed(
    ...
    string name,
    string ticker,
    USBSListData listData
  );

  modifier onlyGuardian() {
    require(msg.sender == guardian, "USBSFactory: You are not the Guardian");
    _;
  }
}
