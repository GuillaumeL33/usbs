...
//
// BRIVO — US Dollar Bonds Short Token (USBS)
//
pragma solidity 0.8.16;

import "contracts/external/openzeppelin/contracts-upgradeable/token/ERC20/ERC20PresetMinterPauserUpgradeable.sol";
import "contracts/usbs/blocklist/BlocklistClientUpgradeable.sol";
import "contracts/usbs/allowlist/AllowlistClientUpgradeable.sol";
  ...
  SanctionsListClientUpgradeable
{
  ...
  function _beforeTokenTransfer(
    ...
    if (from != msg.sender && to != msg.sender) {
      require(!_isBlocked(msg.sender), "USBS: 'sender' address blocked");
      require(!_isSanctioned(msg.sender), "USBS: 'sender' address sanctioned");
      require(
        _isAllowed(msg.sender),
        "USBS: 'sender' address not on allowlist"
      );
    }

    if (from != address(0)) {
      // If not minting
      require(!_isBlocked(from), "USBS: 'from' address blocked");
      require(!_isSanctioned(from), "USBS: 'from' address sanctioned");
      require(_isAllowed(from), "USBS: 'from' address not on allowlist");
    }

    if (to != address(0)) {
      // If not burning
      require(!_isBlocked(to), "USBS: 'to' address blocked");
      require(!_isSanctioned(to), "USBS: 'to' address sanctioned");
      require(_isAllowed(to), "USBS: 'to' address not on allowlist");
    }
  }
